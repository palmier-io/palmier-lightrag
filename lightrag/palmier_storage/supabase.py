...
class SupabaseChunksStorage(BaseKVStorage):
    ...
    async def get_by_id(self, chunk_id: str) -> Optional[dict]:
        response = self.table.select("*").eq("repository_id", self.repo_id).eq("chunk_id", chunk_id).execute()
    ...
    async def get_by_ids(self, chunk_ids: list[str], fields: Optional[set[str]] = None) -> list[Optional[dict]]:
        response = self.table.select("*").eq("repository_id", self.repo_id).in_("chunk_id", chunk_ids).execute()
    ...
    async def get_by_field(self, field: str, values: list) -> dict[str, dict]:
        ...
        if field == "full_doc_id":
            ...
            response = self.table.select("*").eq("repository_id", self.repo_id).in_("full_doc_id", values).execute()
        else:
            # Fallback to filtering metadata for other fields
            response = self.table.select("*").eq("repository_id", self.repo_id).execute()
    ...
    async def filter_keys(self, chunk_ids: list[str]) -> set[str]:
        ...
        existing = self.table.select("chunk_id").eq("repository_id", self.repo_id).in_("chunk_id", chunk_ids).execute()
    ...
    async def delete_by_ids(self, chunk_ids: list[str]):
        self.table.delete().eq("repository_id", self.repo_id).in_("chunk_id", chunk_ids).execute()
