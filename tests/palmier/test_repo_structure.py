...
def test_typescript_skeleton():
    ...
    assert "const processData = (inputData: string[]): Record<string, number> => {" in tsx_skeleton
...
def test_golang_skeleton():
    ...
    assert "func (dp *DefaultProcessor) ProcessData(data []string) (map[string]interface{}, error) {" in skeleton
